            ...
            <div class="col-md-8">
                ...
                <div class="post-container">
                  ...
                  <?php                  
                    ...
                    $offset=($page-1)*$limit;
                    $sql="SELECT post.post_id, post.title, post.description, post.post_date, post.post_img, post.author,  post.category, category.category_name, user.username FROM post
                    LEFT JOIN category ON post.category=category.category_id
                    LEFT JOIN user ON post.author=user.user_id 
                    WHERE post.title LIKE '%{$search_term}%' OR post.description LIKE '%{$search_term}%'
                    ORDER BY post.post_id DESC LIMIT {$offset},{$limit}";
                    $result=mysqli_query($conn, $sql) or die("Query Failed:cat");
                    ...
                    // $sql2="SELECT * FROM user WHERE user_id={$search_term}";
                    // $result2=mysqli_query($conn, $sql2) or die("Query Failed!:sql2");
